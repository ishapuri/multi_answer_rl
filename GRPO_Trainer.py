...
from trainer_utils import (nanstd, RepeatSampler, nanmin, nanmax, shuffle_tensor_dict,split_tensor_dict, 
disable_dropout_in_model,profiling_decorator,profiling_context,entropy_from_logits,split_pixel_values_by_grid,unsplit_pixel_values_by_grid,ensure_master_addr_port,
truncate_with_protected_tokens,print_prompt_completions_sample, identity, shuffle_sequence_dict)
...
class GRPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.rm_path = args.reward_model
        ...
        if self.rm_path is not None:
            reward_config = AutoConfig.from_pretrained(
                    self.rm_path
                )
            architecture = getattr(transformers, reward_config.architectures[0])
            self.reward_model = architecture.from_pretrained(self.rm_path)
            self.reward_tokenizer = AutoTokenizer.from_pretrained(self.rm_path)
            if self.reward_tokenizer.pad_token_id is None:
                self.reward_tokenizer.pad_token = 128004
                # The reward model computes the reward for the latest non-padded token in the input sequence.
                # So it's important to set the pad token ID to the padding token ID of the processing class.
        ...
        # if self.reward_model is not None:
            ...
            #     self.reward_model = prepare_deepspeed(self.reward_model, self.accelerator)
            # else:
            # self.reward_model = self.accelerator.prepare_model(self.reward_model, evaluation_mode=True, device_placement=True)
            #Put into eval mode
            # self.reward_model.eval()
    ...
    def _get_per_token_logps_and_entropies(
        ...
        batch_size = self.args.per_device_train_batch_size # Chunk inputs into smaller batches to reduce memory peak
        if mode == "eval":
            batch_size = 1
        all_logps = []
        all_entropies = []
